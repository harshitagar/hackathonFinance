...
kony.sync.customHttpHeaders = "customhttpheaders";
...
kony.sync.checkForRetryErrorCodes = function(key){
	...
	var suppliedErrorCodes = kony.sync.currentSyncConfigParams[kony.sync.retryErrorCodes];
	if(!kony.sync.isNull(suppliedErrorCodes)) {
		return kony.sync.contains(suppliedErrorCodes, key);
	} else if(key == 1011){
		kony.sync.isSyncStopped = true;
		return false;
	}
	else{
		return kony.sync.contains([1000, 1012, 1013, 1014, 1015], key);
	}
};

kony.sync.eligibleForRetry = function(opstatus, info){
	sync.log.trace("Entering kony.sync.eligibleForRetry");
	var  noOfOfRetriesDefined = kony.sync.currentSyncConfigParams[kony.sync.numberOfRetriesKey];
	// Stop sync if network is not available and developer has not defined retries for opstatus 1011
	// This is done to avoid function stack overflow errors
	if(!kony.sync.isNullOrUndefined(opstatus) && opstatus != 0) {
		if(kony.sync.isNull(noOfOfRetriesDefined)) {
			kony.sync.isSyncStopped = true;
		} else {
			return info > 0 && kony.sync.checkForRetryErrorCodes(opstatus);
		}
	}
	return false;
};
                ...
                function createNewHttpRequest(sessionid, paramsHttpheaders, localRequestCallback, url, timeoutValue) {
                    ...
                    if (paramsHttpheaders !== null) {
                        for (var key in paramsHttpheaders) {
                            httprequest.setRequestHeader(key.toLowerCase(), paramsHttpheaders[key]);
                        }
                    }
                    // [MFSDK-4766] Adding additional headers to the network call, which will be provided by application developer as part of startSession() API call
                    var globalNetworkHeaders = kony.sync.currentSyncConfigParams[kony.sync.customHttpHeaders];
                    if(kony.sync.isValidJSTable(globalNetworkHeaders)){
                        for (var customHeader in globalNetworkHeaders) {
                            httprequest.setRequestHeader(customHeader.toLowerCase(), globalNetworkHeaders[customHeader]);
                        }
                    }
